...
-- Creates a Unity Canvas with CanvasScaler and GraphicRaycaster.
-- Optionally registers it as a UBUI space.
UBUI.NewCanvas(string name, table options) returns table

options:
    string or LuaTransform or LuaGameObject or Element parent
    string space
    string or number renderMode
    number sortingOrder
    boolean overrideSorting
    string sortingLayerName
    boolean pixelPerfect
    boolean scaler
    boolean raycaster
    string or number uiScaleMode
    number referenceWidth
    number referenceHeight
    number matchWidthOrHeight
    number referencePixelsPerUnit

local myCanvas = UBUI.NewCanvas("MyCanvas", {
    space = "mycanvas",
    sortingOrder = 500,
    referenceWidth = 1920,
...
-- Destroys a canvas created by UBUI.NewCanvas and unregisters its space.
UBUI.DestroyCanvas(table canvasHandle)

UBUI.DestroyCanvas(myCanvas)

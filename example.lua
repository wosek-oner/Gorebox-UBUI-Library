...
local MENU_SCENE = "sys_Menu"
local CANVAS_PATH = "GameMaster/GameCanvas-3"
...
local function CanvasReady()
    if UBUI == nil then return false end
    return UBUI.ResolvePath(CANVAS_PATH) ~= nil
end
...
function Update()
    ...
    if scene == MENU_SCENE then
        if built then Reset() end
        return
    end
    ...
    if not built then
        ...
        elseif CanvasReady() then
            if not Build() then buildDelay = 30 end
        end
    end
